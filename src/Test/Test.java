...
public class Test {
	...
	public static void main(String arg[]){
		...
		clienteDAO=(ClienteDAO) FactoryDAO.getInstance(0);
		do{
			try {
				op=showMenuP(stdin);				
				switch(op){
 ...
 */
					try {
						ArrayList<STRUCT> data=clienteDAO.selectAll();
						if(data!=null){
							displayClients(data);
						}else{
...
*/
					...
					try{
						STRUCT cliente=clienteDAO.select(cif);						
						if(cliente!=null){
 ...
 */
					try{
						...
						if(isDiscountValid(desc)){
							if(clienteDAO.updateDiscount(cif, desc)){
	...
	public static boolean listAndSelectTeleUpdate(String cif,Object telef, BufferedReader stdin) throws SQLException, NumberFormatException, IOException{
		...
		if(n<4 && n>0){
			...
			if(num.length()<10){
				...
				aux=new Object[t.length];//pq lo de encima no funciona??
				...
				if(clienteDAO.updatePhones(cif, newTele)){
					result=true;
				}
			}else{
